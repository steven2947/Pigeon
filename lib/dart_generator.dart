...
/// Writes the code for info
void _writeConfigureApi(DartOptions opt, Indent indent, Api api, Root root) {
  assert(api.location == ApiLocation.configure);
  indent.addln('');
  indent.write('class ${api.name} ');
  indent.scoped('{', '}', () {
    api.arguments?.forEach((field) {
      indent.writeln('static ${field.type.baseName} ${field.name} = ${field.value};');
    });
  });
}
...
\t\t: null'''
  ...
  void writeApi(Api api) {
    if (api.location == ApiLocation.host) {
      ...
      _writeConfigureApi(opt, indent, api, root);
    }
  }
