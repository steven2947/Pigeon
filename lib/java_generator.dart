...
void _writeConfigureAPI(Indent indent, Api api, Root root){
  ...
  indent.scoped('{', '}', () {
    api.arguments?.forEach((field) {
      final String type = _javaTypeForDartType(field.type);
      indent.writeln('public static final $type ${field.name} = ${field.value};');
    });
  });
}
...
void generateJava(JavaOptions options, Root root, StringSink sink) {
  ...
  void writeApi(Api api) {
    ...
    } else if (api.location == ApiLocation.configure) {
      _writeConfigureAPI(indent, api, root);
    }
  }
